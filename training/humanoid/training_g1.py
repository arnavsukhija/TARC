...
# G1 joystick command velocity limits: [max_lin_x, max_lin_y, max_ang_z]
# Humanoid walks slower than quadruped; Go1 uses [1.5, 0.8, 2*pi]
G1_COMMAND_LIMITS = [1.0, 0.5, jnp.pi]
...
def experiment(env_name: str = 'G1JoystickFlatTerrain',
    ...
    g1_env = registry.load(env_name, env_cfg)
    ...
    if switch_cost_wrapper:
        ...
        if perturb and hasattr(env_cfg, 'pert_config'):
            env_cfg.pert_config.enable = True
        if hasattr(env_cfg, 'command_config'):
            env_cfg.command_config.a = G1_COMMAND_LIMITS
        ...
        for i in seeds:
            ...
            while env_steps < env_cfg.episode_length:
                ...
                cmd_scale = abs(command[0]) / G1_COMMAND_LIMITS[0] if G1_COMMAND_LIMITS[0] > 0 else 1.0
