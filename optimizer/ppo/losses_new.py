...
Adaptation of Mbpo ppo loss function to work with asymmetric actor critic setup
"""
...
class PPOLoss:
    ...
    def loss(self,
        ...
        if self.non_equidistant_time:
            ...
            steps_for_action = ((t_upper - t_lower) / 2 * pseudo_time_for_action + (t_upper + t_lower) / 2)
            steps_for_action = jnp.floor(steps_for_action).astype(jnp.int32)
            discounting = self.discounting ** steps_for_action
